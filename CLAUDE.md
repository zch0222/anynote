...
| 5 | 前端完全重写（TipTap + BFF + TanStack Query） | 🟡 **代码完成，发版待定** — M0-M7 已于 2026-09-11 全部 `--no-ff` 合并 `dev`。**M8（协同 + 桌面 + E2E/性能 + 收尾）同日完成**：`apps/collab` 自建协同服务 + `/docs` 协同文档库、`apps/desktop` Tauri 骨架与 `/api/auth/exchange` 令牌交换、Playwright 用例（2026-09-12 笔记模块重构后扩充，2026-09-16 UI 补稿后 **118** 条，2026-09-16 知识库概览还原后 **126** 条，2026-09-19 官网首页 D-19/M-14 落地后 **136** 条）、Lighthouse（性能 97-100 / 无障碍 100）、产物预算全绿；全仓前端单测 **1814**（2026-09-19 官网首页后，含落地页 26 条与中间件公开页分流 6 条）+ 协同服务 **75**。**未做**：桌面构建验证（本机无 Rust/MSVC 工具链）、删除 `apps/web-legacy/`（M7.6 的 5 个后端缺口未解，新前端还不能替代它）、合并 `main` 与打 tag `v0.6.0`。AI 流式与 PDF 上传仍受后端阻塞（见里程碑 M7.6）。详见 `docs/refactor/FRONTEND_MILESTONES.md` M7/M7.6/M8/M8.6/§5 与 `docs/refactor/TASKS.md` Phase 5；E2E 与性能门禁命令见 README「测试」 |
...
3. `pnpm --filter web test:e2e` 20 条用例在生产环境全绿
...
- `docs/mobile/` — 移动端适配：`MOBILE_PLAN.md`（技术方案）、`MOBILE_MILESTONES.md`（M10.0–M10.5 进度 + 验收记录 + 与方案的偏差）、`UI_INVENTORY.md`（开工前逐页核对证据）。**M10.0–M10.4 已实现**（2026-09-12，分支 `feat/mobile-foundation`，未并 `dev`）：21 条 `/m/*` 路由落在 `apps/web` 的 `app/(mobile)/m/**` 路由段、不新开应用（认证 Cookie 是 host-only + `sameSite=strict`，刷新锁是进程级）；入口做 UA 分流（`?desktop=1` 逃生口 + 非身份的 `anynote_view` 偏好 Cookie），登录后落 `/m/dashboard`。**2026-09-16（M12 UI 补稿）补跑了移动端 E2E 与 Lighthouse**：`mobile-*.spec.ts` 41 条用例全绿、`lighthouse:budget:mobile` 5 条路由全达标（login 93 / m-dashboard 89 / m-notes 87 / m-docs 85 / m-ai-chat 89，门槛 85）——其中 `/m/dashboard` 修前 74 未达标，根因是 CLS 0.286，修后 0.024。**仍未做**：真机验收（见里程碑「验收记录」）。契约登记见 `.claude/openspec/changes/2026-09-12-mobile-route-segment.md`，逐文件清单见 `docs/changelist/2026-09-12-mobile-adaptation.md`
...
- `docs/ui/` — UI 补稿（里程碑 M12）：`UI补稿设计图.md`（**设计依据**，2026-09-16 定稿：10 个页面 / 40 块画板 / 662 条图例，D-xx 桌面、M-xx 移动、F-xx 跳转流程、Q-xx 规范核对；**2026-09-19 并入官网首页 D-19 / M-14**，来自独立画布「Anynote 官网首页设计」，浅深各一版，编号表相应变为 D 19 / M 14）+ `ui-supplement/`（2 倍分辨率 PNG，屏幕类画板另附深色版）+ `UI补稿落地改造方案.md`（**落地依据**：逐任务写画板 / 文件 / 要点 / 测试 / 验收，附录 A 是 162 条改动的逐条落点、附录 B 是第 4 轮 5 块新画板的完整图例）。**视觉数值以画板图例为准**，方案只写代码怎么落。范围决定见方案 §1.3（慕课/任务只属于知识库、删 `/wikis`、输入框圆角 10、AI 页面暂不出稿），设计稿与后端事实的 5 处冲突及处理见 §1.4。两份实拍核对报告：**`UI检查报告.md`（2026-09-16）**：31 块屏幕画板全部用真实浏览器实拍、与画板同几何裁剪后并排比对的结果，含 21 高 / 24 中 / 11 低的发现清单、路由与动作走查、以及一条重要结论——**多数发现不在附录 A 的 162 条里**（卡片外壳、H1 字阶、列宽、表格列数、列表列头等属于"画板上有、图例没编号"的盲区），逐项修复清单见 `docs/changelist/2026-09-16-ui-audit-fixes.md`；**`2026-09-19-移动端设计还原度核对报告.md`**：对**线上** `/m/*` 在 390×844 下的逐页核对（结论是视觉还原度中等、尚未达标），实拍证据在同名目录 `mobile-audit-2026-09-19/`，**该批 V01–V22 已于 2026-09-20 全部修复并经真实浏览器逐页复验**（含两条核对清单外的追加修复：移动端分段控件全圆化、搜索设置行图标映射），逐文件清单见 `docs/changelist/2026-09-20-mobile-audit-fixes.md`。配套审计脚本 `apps/web/scripts/ui-audit-{capture,zoom}.mjs`（前者修掉了原 `ui-supplement-compare.mjs` 未切换主题、深色截图恒等于浅色的问题）
