...
| 5 | 前端完全重写（TipTap + BFF + TanStack Query） | 🟡 **代码完成，发版待定** — M0-M7 已于 2026-09-11 全部 `--no-ff` 合并 `dev`。**M8（协同 + 桌面 + E2E/性能 + 收尾）同日完成**：`apps/collab` 自建协同服务 + `/docs` 协同文档库、`apps/desktop` Tauri 骨架与 `/api/auth/exchange` 令牌交换、Playwright 用例（2026-09-12 笔记模块重构后扩充，2026-09-16 UI 补稿后 **118** 条，2026-09-16 知识库概览还原后 **126** 条）、Lighthouse（性能 97-100 / 无障碍 100）、产物预算全绿；全仓前端单测 **1773**（2026-09-16 UI 还原度修复后）+ 协同服务 **75**。**未做**：桌面构建验证（本机无 Rust/MSVC 工具链）、删除 `apps/web-legacy/`（M7.6 的 5 个后端缺口未解，新前端还不能替代它）、合并 `main` 与打 tag `v0.6.0`。AI 流式与 PDF 上传仍受后端阻塞（见里程碑 M7.6）。详见 `docs/refactor/FRONTEND_MILESTONES.md` M7/M7.6/M8/M8.6/§5 与 `docs/refactor/TASKS.md` Phase 5；E2E 与性能门禁命令见 README「测试」 |
...
- `docs/ui/` — UI 补稿（里程碑 M12）：`UI补稿设计图.md`（**设计依据**，2026-09-16 定稿：10 个页面 / 40 块画板 / 662 条图例，D-xx 桌面、M-xx 移动、F-xx 跳转流程、Q-xx 规范核对）+ `ui-supplement/`（2 倍分辨率 PNG，屏幕类画板另附深色版）+ `UI补稿落地改造方案.md`（**落地依据**：逐任务写画板 / 文件 / 要点 / 测试 / 验收，附录 A 是 162 条改动的逐条落点、附录 B 是第 4 轮 5 块新画板的完整图例）。**视觉数值以画板图例为准**，方案只写代码怎么落。范围决定见方案 §1.3（慕课/任务只属于知识库、删 `/wikis`、输入框圆角 10、AI 页面暂不出稿），设计稿与后端事实的 5 处冲突及处理见 §1.4。**`UI检查报告.md`（2026-09-16）**：31 块屏幕画板全部用真实浏览器实拍、与画板同几何裁剪后并排比对的结果，含 21 高 / 24 中 / 11 低的发现清单、路由与动作走查、以及一条重要结论——**多数发现不在附录 A 的 162 条里**（卡片外壳、H1 字阶、列宽、表格列数、列表列头等属于"画板上有、图例没编号"的盲区）。逐项修复清单见 `docs/changelist/2026-09-16-ui-audit-fixes.md`；配套审计脚本 `apps/web/scripts/ui-audit-{capture,zoom}.mjs`（前者修掉了原 `ui-supplement-compare.mjs` 未切换主题、深色截图恒等于浅色的问题）
