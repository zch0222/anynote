...
flowchart TB
    ...
    subgraph host[Docker 主机：仅给外部 Nginx 发布入口]
        ...
        subgraph network[Compose bridge：anynote-net]
            ...
            minio[MinIO :9000<br/>私有桶 + 分片直传]
            data[MySQL · Redis · Elasticsearch<br/>RocketMQ · XXL-Job · Logstash]
            volume[(命名数据卷<br/>含 collab-data · minio-data)]
            web -->|HTTP / Cookie 转 Bearer| gateway
            gateway -->|Docker 内网 HTTP| services
            gateway -.-> nacos
            services -.-> nacos
            services -->|内网 endPoint| minio
            services --> data
            minio --> volume
            data --> volume
            collab --> volume
